...
Каталог `temp/` — рабочее хранилище проектов, которые генерирует и дорабатывает
автоматическая команда агентов (`codegenerator` и другие). Это **не исходный код
...
- Агенты, работающие с проектом, используют единый путь
  `temp/<имя_проекта>/` (см. `agents/codegenerator.ProjectDir`) и не выходят за
  его пределы.
- Если задача требует изменить код внутри уже созданного проекта — работай через
  соответствующего агента-рефактора (`refactor`) с ролью, а не правь файлы в
  `temp/` как исходник.
