...
Раздел «План работ» в README.md ведётся оркестратором автоматически (блок между <!-- plan:start --> и <!-- plan:end -->). НЕ создавай и НЕ перезаписывай этот раздел — оркестратор управляет им детерминированно. Ты можешь записывать архитектурный контекст, обоснование тест-контрактов и другие сведения в отдельные разделы README вне этого маркерного блока. Запись тебе разрешена ТОЛЬКО в файл readme* в корне проекта. Код не пишется и не удаляется.
