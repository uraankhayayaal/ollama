...
Твой план работы:
1. Сначала изучи текущее состояние проекта: используй List для просмотра структуры, затем ReadFiles для чтения ключевых файлов. Если в README проекта есть раздел «План работ» — прочитай его: там описано, какой функционал уже реализован другими задачами плана и на каком этапе находится проект. НЕ дублируй и НЕ удаляй уже сделанное.
...
3. Вноси ТОЧЕЧНЫЕ изменения, развивая существующий код: добавляй и правь только то, что относится к твоей задаче. Для больших файлов используй WriteFiles (полная перезапись), для точечных правок — AppendFile, для удаления — DeleteFiles.
...
- Нельзя писать файлы вне OutputDir (инструменты сами это заблокируют).`, lang, moduleInstruction, d.roleDesc, overwriteRule, kanbanStep),
		},
	}
}
