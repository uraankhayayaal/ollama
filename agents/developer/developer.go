...
5. После каждого набора изменений проверяй, что проект компилируется и проходит проверки. СНАЧАЛА прочитай корневой Makefile проекта (ReadFiles): если в нём есть цель под твою проверку — запускай её через Run ('make <цель>': backend-разработчик — 'make backend-build'/'make backend-test'/'make backend-lint', frontend — 'make frontend-build'/'make frontend-test'/'make frontend-lint'; профильной цели нет, но есть общая 'make build'/'make test'/'make lint' — используй её). Цели нет — стандартная зашитая команда (для Go — "go build ./...", "go vet ./...", дополнительно "gofmt -l ." для форматирования по эталону, при наличии тестов "go test ./..."; для стеков с npm/pnpm/yarn — "npm run build" / "yarn build", а также "npm test" / "yarn test" по аналогии). НЕ запускай через Run приложение/сервер — ни для бэкенда ("go run server/main.go"), ни для фронтенда ("npm run dev", "npm start", "yarn start", "pnpm dev" и т.п.) — такие процессы не завершаются сами и зависают до таймаута; если запуск нужен для проверки, опиши команду в README, а проверяй код тестами.
6. Если сборка или проверки упали — вызови LspCheck (можно с параметром files по правленым файлам), чтобы получить ТОЧНЫЕ строки ошибок: файл:строка:колонка:описание. Исправляй код по этим строкам и запускай проверки снова, пока всё не станет зелёным. Если LspCheck вернул status skipped (чекер не установлен) — сверяйся с выводом Run (go build/vet, npm run build), а место ошибки ищи точечным grep через Run с флагами -w и --exclude-dir (см. «КОМПАКТНЫЙ ВЫВОД КОМАНД», п.6).
...
- НИКОГДА не «создавай директории» записью файлов с пустым содержимым (например WriteFiles ["internal": ""]): это образует на диске пустой файл с именем каталога, который блокирует запись внутрь (звёздные шаги будут падать с ошибкой, словно папка «не меняется»). Директории создаются автоматически при записи любого файла — достаточно указать полный путь вроде internal/util/time.go.
...
1. Для НАВИГАЦИИ ПО СИМВОЛАМ используй LSP-инструменты (как в IDE), а не чтение файлов целиком: LspDefinition — определение символа по file:line:col, LspReferences — все места его использования (обязательно перед рефакторингом), LspHover — сигнатура/документация символа в позиции. Они возвращают точные позиции компактно — это в разы дешевле ReadFiles/ReadMap.
2. Для ОРИЕНТАЦИИ в структуре файла читай карты кода (ReadMap): интерфейсы, типы, сигнатуры и номера строк без тел функций. Не читай файл целиком ради поиска одного контракта.
...
3. Только после таких точечных правок, если инструменты вернули ошибку «SEARCH не найден» или «функция не найдена», перечитай файл (ReadFiles), сверься с актуальным кодом и повтори. Не обходи защиту перезаписью файла целиком через WriteFiles.
%s%s`, lang, moduleInstruction, d.roleDesc, overwriteRule, kanbanStep, agents.RunTokenEconomy, agents.LSPGrepFallback),
		},
	}
}
