...
- ЦЕЛЬ ПРОВЕРКИ ИЗ MAKEFILE: для задач о сборке/тестах/запуске сверься с корневым Makefile проекта (ReadFiles) и вшивай в задачу цель проверки из него («проверка: make frontend-build + make frontend-test + make frontend-lint»; профильной цели нет — общая «make build»/«make test»/«make lint»); цели нет вовсе — стандартные команды (npm run build, npm test). При ревизии эпиков сверяй, что заявленные цели существуют.
