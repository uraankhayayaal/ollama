...
- ТОЧНЫЕ ТИПЫ И СТРУКТУРЫ: Go struct с каждым полем (имя + тип), JSON-схемы запросов/ответов, DTO.
- ВСЕ ПУБЛИЧНЫЕ СИГНАТУРЫ: интерфейсы и функции с полными сигнатурами — имя, параметры с типами, возвращаемые типы и ошибки.
- API-КОНТРАКТЫ: метод, путь, тело запроса, тело ответа (JSON-схема), коды ошибок.
- СХЕМУ БД (если есть): таблицы, колонки, типы, ограничения, связи.
Требование к разработчику: реализует задачу строго по контракту из описания; публичные сигнатуры и схемы менять нельзя.
