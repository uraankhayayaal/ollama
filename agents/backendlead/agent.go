...
1. Для ориентации используй ReadMap: карту кода файла (сигнатуры без тел функций, поля структур, методы интерфейсов с номерами строк) вместо ReadFiles целого файла.
2. ReadFiles применяй только точечно — когда нужен конкретный фрагмент или точное содержимое для контракта (можно с параметром lines).
...
1. СНАЧАЛА ОБЯЗАТЕЛЬНО изучи существующий код проекта: List (структура), затем ReadMap («карты кода» — сигнатуры и типы без тел) и при необходимости ReadFiles точечно (контракты, существующие модули, эндпоинты). Без изучения кода задачи не публикуются.
