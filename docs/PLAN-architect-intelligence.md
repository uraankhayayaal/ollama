...
- [ ] `tools/stacktool.go`: `DetectStack` (stackdetect + эвристика состава);
...
- [ ] `toolNames` += `DetectStack`; промпт: «Сначала DetectStack; назначай
      эпики только нужным лидам; при неясном стеке — AskUser»
- [ ] Промпт: убрать жёсткий «всегда Go+React+4 лида»; для существующего
      проекта стек берётся из кода
- [ ] Тест (fake provider): «консольное приложение на Go» → бэклог не
      содержит эпиков Frontend Lead

### Ф-3 — Глубина декомпозиции + смежные системы
- [ ] Промпт, секция «Глубина декомпозиции» (слои UI→API→модель→тесты→деплой;
      KISS-минимум, а не «просто кнопка»)
- [ ] Промпт, секция «Исследование смежного функционала»: перед эпиком-
      изменением CodeSearch/LSP/ReadFiles → список затрагиваемых модулей в
      description (+ зависимости)
- [ ] У архитектора `toolNames` уже есть LSP; при необходимости расширить
      чтение `BoardGetEpic`/`BoardGetTask` (Ф-1) промптом «смотри детали перед
      правкой контрактов»
- [ ] Hermetic-тест промпта: грepируется наличие фраз «слои», «CodeSearch»,
...
- [ ] `agents/planner/kanban.go`: фаза `phaseArchitectReview` (в `runPhases`
...
- [ ] Промпт архитектора (режим ревью): «проанализируй эпики, созданные
      в чате: корректность ТЗ, стек (Р-3), глубина (Р-4), смежные модули;
      скорректируй `BoardUpdateEpic` ДО декомпозиции»
- [ ] Тесты: (1) у чат-ассистента нет task-write инструментов (запрос на
