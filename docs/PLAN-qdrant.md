...
- [ ] `rag/chunk.go`: структурная нарезка (функции/методы/структуры Go через
      ...
      юнит-тесты на куски
- [ ] `main.go`: команда `go run . index <имя_проекта>` — обход дерева
...
- [ ] `rag/index.go`: upsert точек с payload (`file_path`/`start_line`/`end_line`/
      `code_content`/`scope`), удаление старых векторов файла при переиндексации
- [ ] Тесты: fake-клиент, содержимое payload, инкрементальность (повторный
...
- [ ] `agents/planner/ragcontext.go`: в `BuildProjectMap` — семантическая
      ...
      в карте (в рамках `maxMapChars`)
- [ ] Scope-фильтры шагов: связка `scope` планировщика → фильтр Qdrant
      (по образцу `lspgate.go` `scopeSourceFiles`)
- [ ] Тесты: с RAG и без (фолбэк на текущую карту), лимиты не превышаются

### Ф-5 — Авто-обновление при изменении кода
- [ ] `runner/reindex.go`: после мутирующих инструментов переиндексировать
      `FileOps.touched` (переиспользуем хук `runner/autofix.go`); управление —
      env-флаг (по умолчанию `0`)
- [ ] Тесты: мутация → переиндекс затронутых файлов, отсутствие дублей,
      выкл флагом
- [ ] Документация в `readme.md` (env-таблица RAG)
