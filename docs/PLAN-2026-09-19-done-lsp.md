...
Обновлять этот файл по мере выполнения (чекбоксы `[x]`), как в `PLAN-2026-09-17-done-webui.md`.
...
| `LSP_BIN_PATH` | `` | Дополнительные каталоги поиска чекеров/серверов (высший приоритет); без него бинарники ищутся в PATH + типовых каталогах установки (`~/go/bin`, GOBIN/GOPATH, префиксы npm/nvm, Homebrew) через `tools/binpath` |
...
| `docs/PLAN-2026-09-19-done-lsp.md` | Этот план |
...
1. Промпты агентов обучают LSP-навигации (LspDefinition/LspReferences/LspHover) вместо
   чтения файлов целиком: developer, backendlead/frontendlead, qalead, qaengineer,
   architect, planner, chatassist. Поиск бинарников не ограничивается PATH процесса:
   `tools/binpath` (типовые каталоги установки + `LSP_BIN_PATH`) — серверы/чекеры
   находятся, даже если PATH демона урезан.
