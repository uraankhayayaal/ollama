...
- [ ] `runner/autofix.go`: хук после прохода вызовов раунда — LspCheck по
      `touched`, скрытый user-промпт, счётчик `LSP_MAX_FIX_ROUNDS`, сброс после
      завершения цикла; событие в `runevents` (Web UI видит как tool-вызов)
- [ ] управление фичей: `LSP_AUTO_FIX`/`LSP_MAX_FIX_ROUNDS`; в Web UI — обычный
      tool-таймлайн без новых вьюх
- [ ] Тесты: герметичный fake-агент (диагностики при мутации) → проверка скрытого
      промпта, переисправления и лимита; прогон всего `runner` под `-race`
- [ ] Верификация Ф-2: регресс CLI (автономный режим без LSP не меняет поведение
      — хук выключен при `FileOps.touched` пустом и `LSP_AUTO_FIX=0`)
...
1. Открыть этот файл, взять первый незачёркнутый пункт Ф-1, выполнить,
   отметить `[x]`, закоммитить (`docs/` + код).
